 ...
 */
@interface BbdRCTHTTPFormDataHelper : NSObject

@property (nonatomic, weak) BbdRCTNetworking *networker;

@end

@implementation BbdRCTHTTPFormDataHelper
{
                                 ...
                                 callback:(RCTHTTPQueryResult)callback
{
  ...
  for (NSUInteger i = 0; i < _parts.count; i++) {
    ...
    if (uri && [[uri substringToIndex:@"ph:".length] caseInsensitiveCompare:@"ph:"] == NSOrderedSame) {
      uri = [BbdRCTNetworkingPHUploadHackScheme stringByAppendingString:[uri substringFromIndex:@"ph".length]];
 ...
 */
@implementation BbdRCTNetworking
{
...
(RCTURLRequestCancellationBlock (^)(NSError *error, NSDictionary<NSString *, id> *result))callback
{
  ...
  for (id<BbdRCTNetworkingRequestHandler> handler in _requestHandlers) {
  ...
  if (request) {
    ...
    BbdRCTNetworkTask *task = [self networkTaskWithRequest:request completionBlock:^(NSURLResponse *response, NSData *data, NSError *error) {
      dispatch_async(self->_methodQueue, ^{
    ...
    __weak BbdRCTNetworkTask *weakTask = task;
    return ^{
      [weakTask cancel];
  ...
  if (formData) {
    BbdRCTHTTPFormDataHelper *formDataHelper = [BbdRCTHTTPFormDataHelper new];
...
- (void)sendData:(NSData *)data
    responseType:(NSString *)responseType
        response:(NSURLResponse *)response
         forTask:(BbdRCTNetworkTask *)task
{
  RCTAssertThread(_methodQueue, @"sendData: must be called on method queue");

  id responseData = nil;
  for (id<BbdRCTNetworkingResponseHandler> handler in _responseHandlers) {
  ...
  if (!responseData) {
    ...
    if ([responseType isEqualToString:@"text"]) {
      ...
      responseData = [BbdRCTNetworking decodeTextData:data fromResponse:task.response withCarryData:nil];
      if (!responseData) {
     ...
     responseSender:(RCTResponseSenderBlock)responseSender
{
  ...
  __block BbdRCTNetworkTask *task;
  ...
  if (incrementalUpdates) {
    if ([responseType isEqualToString:@"text"]) {
      ...
      incrementalDataBlock = ^(NSData *data, int64_t progress, int64_t total) {
        ...
        NSString *responseString = [BbdRCTNetworking decodeTextData:data
...
- (void)addRequestHandler:(id<BbdRCTNetworkingRequestHandler>)handler
{
  if (!_requestHandlers) {
...
- (void)addResponseHandler:(id<BbdRCTNetworkingResponseHandler>)handler
{
...
- (void)removeRequestHandler:(id<BbdRCTNetworkingRequestHandler>)handler
{
  [_requestHandlers removeObject:handler];
}

- (void)removeResponseHandler:(id<BbdRCTNetworkingResponseHandler>)handler
{
  [_responseHandlers removeObject:handler];
}

- (BbdRCTNetworkTask *)networkTaskWithRequest:(NSURLRequest *)request completionBlock:(RCTURLRequestCompletionBlock)completionBlock
{
  ...
  BbdRCTNetworkTask *task = [[BbdRCTNetworkTask alloc] initWithRequest:request
                                                         handler:handler
...
- (BbdRCTNetworking *)bbdNetworkingMobule
{
  return [self moduleForClass:[BbdRCTNetworking class]];
}
