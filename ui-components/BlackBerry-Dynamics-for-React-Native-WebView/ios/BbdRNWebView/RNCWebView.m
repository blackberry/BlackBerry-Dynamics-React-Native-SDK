...
static NSURLCredential *acceptedCertificateCredential;
static NSString *failedSSLHost;
static NSError *serverTrustError;
                  ...
                  completionHandler:(void (^)(NSURLSessionAuthChallengeDisposition disposition, NSURLCredential * _Nullable))completionHandler
{
    ...
    {
        if (acceptedCertificateCredential)
        {
            completionHandler(NSURLSessionAuthChallengeUseCredential, acceptedCertificateCredential);
        }
        else if ([[challenge protectionSpace] serverTrust] != nil && customCertificatesForHost != nil && host != nil)
        {
                     ...
                     withError:(NSError *)error
{
  if (_onLoadingError) {
    ...
    {
      acceptedCertificateCredential = nil;
      [self displayServerTrustDialogFromError:error];
      return;
    }
...
-(void)trustServerCertificate
{
    ...
    acceptedCertificateCredential = [NSURLCredential credentialForTrust:trust];
#if !defined (DISABLE_GD)
    acceptedCertificateCredential.gdPersistence = NSURLCredentialPersistencePermanent;
#endif
}

-(void)showUntrustedResourceError:(NSError *)error
{
    NSMutableDictionary<NSString *, id> *event = [self baseEvent];
    [event addEntriesFromDictionary:@{
      @"didFailProvisionalNavigation": @YES,
      @"domain": error.domain,
      @"code": @(error.code),
      @"description": @"Connection to untrusted resource is denied",
    }];
    _onLoadingError(event);
}

-(void)proceedLoadingRequest
{
    [_webView loadRequest:[self requestForSource:self.source]];
}

-(void)displayServerTrustDialogFromError:(NSError *)trustError
{
    NSString *alertMessage = trustError.localizedRecoverySuggestion;
    NSString *alertTitle = trustError.localizedDescription;

    serverTrustError = trustError;
    failedSSLHost = [trustError.userInfo objectForKey:NSURLErrorFailingURLStringErrorKey];
    ...
    if (alertMessage.length) {
      ...
      __weak typeof(self) weakSelf = self;
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:@"Deny"
                                                            style:UIAlertActionStyleCancel
                                                          handler:^(UIAlertAction * _Nonnull action) {
                                                                failedSSLHost = nil;
                                                                [weakSelf showUntrustedResourceError:serverTrustError];
                                                                [weakAlertController
                                                                    dismissViewControllerAnimated:YES
                                                                                       completion:nil];
                                                                }]];

          [alertController addAction:[UIAlertAction actionWithTitle:@"Always trust"
                                                              style:UIAlertActionStyleDefault
                                                            handler:^(UIAlertAction * _Nonnull action) {
                                                                  [weakSelf trustServerCertificate];
                                                                  [weakSelf proceedLoadingRequest];
                                                                  [weakAlertController
                                                                      dismissViewControllerAnimated:YES
                                                                                         completion:nil];
                                                                  }]];
