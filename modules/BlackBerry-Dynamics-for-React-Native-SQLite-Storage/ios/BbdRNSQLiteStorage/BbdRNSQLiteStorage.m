...
@implementation RNReactNativeBbdStorage

RCT_EXPORT_MODULE(RNReactNativeBbdStorage);
...
- (id) init
{
    ...
    if (self) {
        ...
        NSError *err;
        if ([[GDFileManager defaultManager] fileExistsAtPath: nosync])
        {
        ...
        {
            if ([[GDFileManager defaultManager] createDirectoryAtPath: nosync withIntermediateDirectories:NO attributes: nil error:&err])
            {
...
RCT_EXPORT_METHOD(echoStringValue: (NSDictionary *) options success:(RCTResponseSenderBlock)success error:(RCTResponseSenderBlock)error)
{
    ...
    SQLiteResultBbd* pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_OK messageAsString:string_value];
...
{
    SQLiteResultBbd* pluginResult = nil;
    NSString *dbname;
    ...
    @synchronized (self) {
        ...
        if (dbfilename == NULL) {
            ...
            pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_OK messageAsString:@"You must specify database name"];
        } else {
            ...
            if (dbInfo != NULL && dbInfo[@"dbPointer"] != NULL) {
                ...
                pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_OK messageAsString:@"Database opened"];
            } else {
                ...
                if (assetFilePath != NULL && assetFilePath.length > 0) {
                    @try {
                        if ([assetFilePath isEqualToString:@"1"]){
                            ...
                            NSURL * documentsDirUrl = [[[GDFileManager defaultManager] URLsForDirectory:NSDocumentDirectory
                ...
                if (options[@"readOnly"] && assetFilePath != NULL){
                    ...
                    if (![[GDFileManager defaultManager] fileExistsAtPath:dbname] && assetFilePath != NULL) {
                ...
                if (sqlite3enc_open_v2(name, &db,sqlOpenFlags, NULL) != SQLITE_OK) {
                    pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsString:@"Unable to open DB"];
                    return;
                } else {
                    ...
                    if(sqlite3_exec(db, (const char*)"SELECT count(*) FROM sqlite_master;", NULL, NULL, NULL) == SQLITE_OK) {
                        ...
                        pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_OK messageAsString: msg];
                        RCTLog(@"%@", msg);
                    } else {
                        NSString *msg = [NSString stringWithFormat:@"Unable to open %@", (key != NULL) ? @"secure database with key" : @"database"];
                        pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsString:msg];
                        RCTLog(@"%@", msg);
                        sqlite3_close (db);
...
RCT_EXPORT_METHOD(close: (NSDictionary *) options success:(RCTResponseSenderBlock)success error:(RCTResponseSenderBlock)error)
{
    ...
    @synchronized (self) {
        ...
        if (dbFileName == NULL) {
            ...
            pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsString:@"You must specify database path"];
        } else {
            NSDictionary *dbInfo = openDBs[dbFileName];
            if (dbInfo == NULL || dbInfo[@"dbPointer"] == NULL) {
                RCTLog(@"close: db name was not found in open databases: %@", dbFileName);
                pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsString:@"Specified db was not open"];
            } else {
                sqlite3 *db = [((NSValue *) dbInfo[@"dbPointer"]) pointerValue];
                NSString *dbPath = dbInfo[@"dbPath"];

                if ([[GDFileManager defaultManager] fileExistsAtPath:dbPath]) {
                ...
                if (db == NULL) {
                    ...
                    sqlite3_close (db);
                    [openDBs removeObjectForKey:dbFileName];
                    pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_OK messageAsString:@"DB closed"];
                }
...
RCT_EXPORT_METHOD(attach: (NSDictionary *) options success:(RCTResponseSenderBlock)success error:(RCTResponseSenderBlock)error)
{
    ...
    if (dbName == NULL || dbAlias == NULL) {
        ...
        pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsString:@"You must specify dbName and dbAlias"];
    } else {
        ...
        @synchronized (self) {
            ...
            if (dbInfo == NULL || dbInfo[@"dbPointer"] == NULL) {
                ...
                pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsString:@"Specified db for ALIAS was not open"];
            } else {
                sqlite3 *db = [((NSValue *) dbInfo[@"dbPointer"]) pointerValue];
                ...
                if(sqlite3_exec(db, [sql UTF8String], NULL, NULL, NULL) == SQLITE_OK) {
                    pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_OK messageAsString:@"Database attached successfully."];
                } else {
                    pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsString:@"Unable to attach DB"];
                }
            }
        }
    }
    ...
    @synchronized (self) {
        if (dbfilename == NULL) {
            ...
            if ([[GDFileManager defaultManager] fileExistsAtPath:dbPath]) {
                ...
                [[GDFileManager defaultManager]removeItemAtPath:dbPath error:nil];
                [openDBs removeObjectForKey:dbfilename];
                pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_OK messageAsString:@"DB deleted"];
            } else {
                RCTLog(@"delete: db was not found: %@", dbPath);
                pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsString:@"The database does not exist on that path"];
            }
        }
    }
...
RCT_EXPORT_METHOD(executeSqlBatch: (NSDictionary *) options success:(RCTResponseSenderBlock)success error:(RCTResponseSenderBlock)error)
{
    ...
    SQLiteResultBbd *pluginResult;

    @synchronized(self) {
        for (NSMutableDictionary *dict in executes) {
            SQLiteResultBbd *result = [self executeSqlWithDict:dict andArgs:dbargs];
        ...
        pluginResult = [SQLiteResultBbd resultWithStatus:SQLiteStatus_OK messageAsArray:results];
    }
...
-(SQLiteResultBbd *) executeSqlWithDict: (NSMutableDictionary*)options andArgs: (NSMutableDictionary*)dbargs
{
    NSString *dbfilename = dbargs[@"dbname"];
    if (dbfilename == NULL) {
        return [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsString:@"You must specify database path"];
    }
    ...
    NSDictionary *dbInfo = openDBs[dbfilename];
    if (dbInfo == NULL || dbInfo[@"dbPointer"] == NULL){
        return [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsString:@"No such database, you must open it first"];
    }
    ...
    if (sql == NULL) {
        return [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsString:@"You must specify a sql query to execute"];
    }
    ...
    if (sqlite3_prepare_v2(db, sql_stmt, -1, &statement, NULL) != SQLITE_OK) {
        error = [RNReactNativeBbdStorage captureSQLiteErrorFromDb:db];
        keepGoing = NO;
    } else if (params != NULL) {
                ...
                while (i < count) {
                    ...
                    switch (column_type) {
                        ...
                        case SQLITE_FLOAT:
                            ...
                            break;
                        case SQLITE_BLOB:
                            columnValue = [RNReactNativeBbdStorage getBlobAsBase64String: sqlite3_column_blob(statement, i)
    ...
    if (error) {
        return [SQLiteResultBbd resultWithStatus:SQLiteStatus_ERROR messageAsDictionary:error];
    }
    ...
    return [SQLiteResultBbd resultWithStatus:SQLiteStatus_OK messageAsDictionary:resultSet];
}
...
+(NSDictionary *)captureSQLiteErrorFromDb:(struct sqlite3 *)db
{
    ...
    int webSQLCode = [RNReactNativeBbdStorage mapSQLiteErrorCode:code];
