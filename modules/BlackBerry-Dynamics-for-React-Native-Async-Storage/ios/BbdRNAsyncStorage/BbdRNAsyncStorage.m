...
{
    NSError *error;
    [[GDFileManager defaultManager] removeItemAtPath:RCTGetStorageDirectory() error:&error];
...
{
    NSDictionary *attributes = [[GDFileManager defaultManager] attributesOfItemAtPath:manifestFilePath error:nil];
...
{
    NSError *error;
    if (![[GDFileManager defaultManager] removeItemAtPath:RCTCreateStorageDirectoryPath(RCTOldStorageDirectory) error:&error]) {
...
static void RCTStorageDirectoryMigrate()
{
    ...
    if (![[GDFileManager defaultManager] copyItemAtPath:RCTCreateStorageDirectoryPath(RCTOldStorageDirectory) toPath:RCTGetStorageDirectory() error:&error]) {
...
static void RCTStorageDirectoryMigrationCheck()
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if ([[GDFileManager defaultManager] fileExistsAtPath:RCTCreateStorageDirectoryPath(RCTOldStorageDirectory) isDirectory:&isDir] && isDir) {
            // Check if the new storage directory location already exists
            if ([[GDFileManager defaultManager] fileExistsAtPath:RCTGetStorageDirectory()]) {
                ...
                if ([RCTManifestModificationDate(RCTGetManifestFilePath()) compare:RCTManifestModificationDate(RCTCreateManifestFilePath(RCTOldStorageDirectory))] == 1) {
                    ...
                    if (![[GDFileManager defaultManager] removeItemAtPath:RCTGetStorageDirectory() error:&error]) {
...
#pragma mark - RNReactNativeBbdAsyncStorage

@implementation RNReactNativeBbdAsyncStorage
{
    BOOL _haveSetup;
...
RCT_EXPORT_MODULE(RNReactNativeBbdAsyncStorage)
...
-(void)_writeToFile:(NSString*)filePath withData:(id)data error:(NSError*)error
{
    if (filePath) {
        ...
        RCTMakeError(@"Path was not passed to the method call", error, nil);
    }
}
...
- (NSDictionary *)_writeEntry:(NSArray<NSString *> *)entry changedManifest:(BOOL *)changedManifest
{
    ...
    if (value.length <= RCTInlineValueThreshold) {
        if (_manifest[key] == (id)kCFNull) {
            ...
            [[GDFileManager defaultManager] removeItemAtPath:filePath error:nil];
