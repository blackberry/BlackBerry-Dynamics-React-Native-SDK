...
readFile(_path2.default.join(__dirname, 'android/app/src/main/res/values/strings.xml')).then(function (data) {
  ...
  _commander2.default.version('2.4.1')
  .arguments('<newBundleId>')
  .option('-n, --appName [value]', 'Set custom app name').action(function (newBundleId) {
    var newName = _commander2.default.appName;
    ...
    var nS_NewName = void 0;
    var lC_Ns_NewAppName = void 0;

    if (newName) {
      nS_NewName = newName.replace(/\s/g, '');
      if (nS_NewName) {
        lC_Ns_NewAppName = nS_NewName.toLowerCase();
      }
    ...
    var resolveFilesToModifyContent = function resolveFilesToModifyContent() {
      return new Promise(function (resolve) {
        ...
        if (listOfFilesToModifyContent.length === 0) {
          resolve();
        } else {
    ...
    var resolveJavaFiles = function resolveJavaFiles() {
      return new Promise(function (resolve) {
        readFile(_path2.default.join(__dirname, 'android/app/src/main/AndroidManifest.xml')).then(function (data) {
          ...
          lC_Ns_NewAppName = lC_Ns_NewAppName ? lC_Ns_NewAppName : lC_Ns_CurrentAppName;
          var newBundleID = bundleID ? bundleID : 'com.' + lC_Ns_NewAppName;
    ...
    var rename = function rename() {
      resolveFoldersAndFiles.then(resolveFilesToModifyContent).then(resolveJavaFiles).then(resolveBundleIdentifiers).then(deletePreviousBundleDirectory).then(cleanBuilds).then(function () {
        return console.log(('APP SUCCESSFULLY RENAMED! \uD83C\uDF89 \uD83C\uDF89 \uD83C\uDF89').green);
      }).then(function () {
