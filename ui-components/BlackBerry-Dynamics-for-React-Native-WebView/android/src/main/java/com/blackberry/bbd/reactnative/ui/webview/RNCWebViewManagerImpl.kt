...
class RNCWebViewManagerImpl {
    ...
    val COMMAND_FOCUS = 8
    val COMMAND_ANSWER_PERMISSION_REQUEST = 9
      ...
      when (commandId) {
        ...
        "answerPermissionRequest" -> {
          if (args == null || args.size() < 1) {
            throw java.lang.RuntimeException("Arguments for answerPermissionRequest are null!")
          }

          val answer = args.getBoolean(0)
          val resources = ArrayList<String>()
          for (i in 1 until args.size()) {
            resources.add(args.getString(i))
          }
          var webChromeClient = webView.webChromeClient as RNCWebChromeClient?
          webChromeClient?.answerPermissionRequest(answer, resources.toTypedArray())
        }
        "loadUrl" -> {
          if (args == null) {
